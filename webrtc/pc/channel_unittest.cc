...
class ChannelTest : public testing::Test, public sigslot::has_slots<> {
 public:
  ...
  void TestSetContentFailure() {
    ...
    EXPECT_FALSE(channel1_->SetLocalContent(&content, cricket::CA_OFFER, &err));
    EXPECT_FALSE(
        channel1_->SetLocalContent(&content, cricket::CA_ANSWER, &err));

    media_channel1_->set_fail_set_send_codecs(true);
    EXPECT_FALSE(
        channel1_->SetRemoteContent(&content, cricket::CA_OFFER, &err));
    media_channel1_->set_fail_set_send_codecs(true);
    EXPECT_FALSE(
        channel1_->SetRemoteContent(&content, cricket::CA_ANSWER, &err));
  }
