...
uint32_t AimdRateControl::ChangeBitrate(uint32_t new_bitrate_bps,
  ...
  switch (rate_control_state_) {
    ...
    case kRcDecrease:
      ...
      if (bitrate_is_initialized_ &&
          new_bitrate_bps < current_bitrate_bps_) {
        const unsigned int decrease = current_bitrate_bps_ - new_bitrate_bps;
        if (decrease > current_bitrate_bps_ / 4) {
          // If bit rate decrease is greater than 25% of current bitrate, it
          ...
          last_decrease_ = rtc::Optional<int>(decrease);
        }
      }
