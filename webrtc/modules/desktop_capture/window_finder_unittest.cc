...
WindowId GetTopWindowWithRect(DesktopRect* rect) {
  ...
  return reinterpret_cast<WindowId>(GetAncestor(top_window, GA_ROOT));
}
...
TEST(WindowFinderTest, FindTopWindow) {
  ...
  if (top_window == NULL) {
    // No top window or top window is too small. Skip this test.
    // This usually won't happen, Windows cannot run console applications
    // without console / cmd windows.
    return;
  }
