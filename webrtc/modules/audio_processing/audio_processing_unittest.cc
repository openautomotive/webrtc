...
TEST(ApmConfiguration, ToggleCapturePostProcessing) {
  ...
  rtc::scoped_refptr<AudioProcessing> apm = AudioProcessing::Create(
      webrtc_config, nullptr, std::move(mock_post_processor));
  ...
  AudioProcessing::Config config;
  const bool post_processor_states[4] = {true, false, false, true};
  for (bool post_processor_enabled : post_processor_states) {
    config.post_processor.enabled = post_processor_enabled;
    apm->ApplyConfig(config);
    EXPECT_CALL(*mock_post_processor_ptr, Process(testing::_))
        .Times(post_processor_enabled ? 1 : 0);
    apm->ProcessStream(audio.get());
    std::cout << "bloop" << std::endl;
  }
}

TEST(ApmConfiguration, HandlesEnableWithoutPostProcessor) {
