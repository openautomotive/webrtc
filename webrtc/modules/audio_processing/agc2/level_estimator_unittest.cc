...
// TODO(alessiob): When the internal params become hard-coded, remove lines
// below.
constexpr float kAttackMs = 1.f;
constexpr float kDecayMs = 20.f;
...
void TestLevelEstimator(int sample_rate_hz, int num_channels,
                        float input_level_linear_scale, float expected_min,
                        float expected_max) {
  ApmDataDumper apm_data_dumper(0);
  LevelEstimator level_estimator(kNumSubFrames, kAttackMs, kDecayMs,
                                 sample_rate_hz, &apm_data_dumper);
...
float TimeMsToDecreaseLevel(int sample_rate_hz, int num_channels,
                            float input_level_db, float level_reduction_db,
                            int num_sub_frames) {
  ...
  ApmDataDumper apm_data_dumper(0);
  LevelEstimator level_estimator(num_sub_frames, kAttackMs, kDecayMs,
                                 sample_rate_hz, &apm_data_dumper);
  ...
  return static_cast<float>(sub_frames_until_level_reduction) *
         kFrameDurationMs / num_sub_frames;
}
}  // namespace
...
TEST(AutomaticGainController2LevelEstimator, TimeToDecreaseForNormalizedLevel) {
  ...
  float time_to_decrease =
      TimeMsToDecreaseLevel(21000, 1, 0, kLevelReductionDb, kNumSubFrames);
...
TEST(AutomaticGainController2LevelEstimator, TimeToDecreaseForFullScaleLevel) {
  ...
  float time_to_decrease =
      TimeMsToDecreaseLevel(25000, 1, 0, kLevelReductionDb, kNumSubFrames);
...
TEST(AutomaticGainController2LevelEstimator,
  ...
  float time_to_decrease = TimeMsToDecreaseLevel(
      27000, kNumChannels, 0, kLevelReductionDb, kNumSubFrames);
...
TEST(AutomaticGainController2LevelEstimator,
     TimeToDecreaseSingleSampleSubFrames) {
  constexpr float kLevelReductionDb = 25;
  constexpr float kExpectedTime = kLevelReductionDb * kDecayMs;
  constexpr size_t kSampleRate = 25000;

  float time_to_decrease =
      TimeMsToDecreaseLevel(kSampleRate, 1, 0, kLevelReductionDb,
                            rtc::CheckedDivExact(kSampleRate * kFrameDurationMs,
